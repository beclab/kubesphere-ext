...
var promQLTemplates = map[string]string{
	...
	"node_device_partition_size_total": `min by (device, instance, node, host_ip, role) ((node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"} * on(namespace, pod) group_left(node, host_ip, role) node_namespace_pod:kube_pod_info:) * on(device, instance, mountpoint) group_left()(label_replace(node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"},"len", "{{len(mountpoint)}}", "", "") / clamp_min(label_replace(node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"},"len", "{{len(mountpoint)}}", "", ""), 1)))`,
