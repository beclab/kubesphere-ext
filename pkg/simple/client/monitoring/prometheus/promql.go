...
var promQLTemplates = map[string]string{
	...
	"cluster_disk_size_usage":            `sum(max(label_replace(node_filesystem_size_bytes{device=~"/dev/.*", device!~"/dev/loop\\d+", job="node-exporter"} - node_filesystem_avail_bytes{device=~"/dev/.*", device!~"/dev/loop\\d+", job="node-exporter"}, "base_device", "$1", "device", "/dev/(nvme\\d+n\\d+|mmcblk\\d+|[a-z]+)p?\\d*") unless on (instance, base_device) label_replace(node_disk_info{removable="1",bus="usb"}, "base_device", "$1", "device", "(.*)")) by (device, instance))`,
	"cluster_disk_size_utilisation":      `cluster:disk_utilization:ratio`,
	"cluster_disk_size_capacity":         `sum(max(label_replace(node_filesystem_size_bytes{device=~"/dev/.*", device!~"/dev/loop\\d+", job="node-exporter"}, "base_device", "$1", "device", "/dev/(nvme\\d+n\\d+|mmcblk\\d+|[a-z]+)p?\\d*") unless on (instance, base_device) label_replace(node_disk_info{removable="1",bus="usb"}, "base_device", "$1", "device", "(.*)")) by (device, instance))`,
	"cluster_disk_size_available":        `sum(max(label_replace(node_filesystem_avail_bytes{device=~"/dev/.*", device!~"/dev/loop\\d+", job="node-exporter"}, "base_device", "$1", "device", "/dev/(nvme\\d+n\\d+|mmcblk\\d+|[a-z]+)p?\\d*") unless on (instance, base_device) label_replace(node_disk_info{removable="1",bus="usb"}, "base_device", "$1", "device", "(.*)")) by (device, instance))`,
	...
	"node_device_size_usage":           `sum by(device, node, role) (node_filesystem_size_bytes{device!~"/dev/loop\\d+",device=~"/dev/.*",job="node-exporter"} * on(namespace, pod) group_left(node, role) node_namespace_pod:kube_pod_info:{$1}) - sum by(device, node, role) (node_filesystem_avail_bytes{device!~"/dev/loop\\d+",device=~"/dev/.*",job="node-exporter"} * on(namespace, pod) group_left(node, role) node_namespace_pod:kube_pod_info:{$1})`,
	"node_device_size_utilisation":     `1 - sum by(device, node, role) (node_filesystem_avail_bytes{device!~"/dev/loop\\d+",device=~"/dev/.*",job="node-exporter"} * on(namespace, pod) group_left(node, role) node_namespace_pod:kube_pod_info:{$1}) / sum by(device, node, role) (node_filesystem_size_bytes{device!~"/dev/loop\\d+",device=~"/dev/.*",job="node-exporter"} * on(namespace, pod) group_left(node, role) node_namespace_pod:kube_pod_info:{$1})`,
	"node_device_partition_size_total": `min by (device, instance, node, role) ((node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"} * on(namespace, pod) group_left(node, role) node_namespace_pod:kube_pod_info:) * on(device, instance, mountpoint) group_left()(label_replace(node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"},"len", "{{len(mountpoint)}}", "", "") / clamp_min(label_replace(node_filesystem_size_bytes{device!~"/dev/loop\\d+", device=~"/dev/.*", job="node-exporter"},"len", "{{len(mountpoint)}}", "", ""), 1)))`,
	"node_filesystem_size_bytes":       `node_filesystem_size_bytes{device!~"/dev/loop\\d+",device=~"/dev/.*",job="node-exporter"}`,
